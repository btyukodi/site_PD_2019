...
def propagate_NOT_THIS(capsid, kT, t_init, t_final, umbrella_window, run_params, RNG, path):
...
def run(run_params, data_root='', init_config=None, run_id=None):
    ...
    #should be read from params {'kT':[1,3,4]}
    kT_values = params['global_params']['kT_values']#np.sort(np.array([1.5, 1.6, 1.7]))#
    capsids = []
    #capsid_shelves = []
    #umbrella_shelves = []
    umbrella_windows = []
    RNGs = []
    for kT in kT_values:
        new_capsid = mc.create_single_subunit_capsid(subunit_type=params['subunits'].keys()[0])
        pr.update_capsid_parameters(new_capsid, params) #only needed if seed will be used
        new_capsid.kT=kT
        capsids.append(new_capsid)

        umbrella_windows.append(mc.UmbrellaWindow(0,9,1))

        RNGs.append(np.random.RandomState())
        

        db = gdbm.open(path+'/tmp/Capsid_kT'+("%.9f" % kT),'n')
        shelf = shelve.Shelf(db)
        shelf.close()

        dbu = gdbm.open(path+'/tmp/Umbrella_hist_kT'+("%.9f" % kT),'n')
        shelf_u = shelve.Shelf(dbu)
        shelf_u.close()
    ...
    umbrella_nsteps = 1000000 #number of steps in an umbrella window
    dt_exchange = 5000
    if (umbrella_nsteps % dt_exchange <>0):
        raise ValueError('Number of umbrella window steps has to be multiple of exchange steps')


    for t1 in xrange(1, tmax/dt_exchange):
        pool = Pool(processes=len(kT_values))
        results = []
        for ix, kT in enumerate(kT_values):
            results.append(pool.apply_async(propagate, [capsids[ix], kT, t1*dt_exchange, (t1+1)*dt_exchange, umbrella_windows[ix], run_params, RNGs[ix], path]))
            #results.append(propagate(capsids[ix], kT, t1*dt_exchange, (t1+1)*dt_exchange, umbrella_windows[ix], run_params, RNGs[ix], path))
    
        #get last capsid snapshots and umbrella windows; passing to function will not modify umbrella windows apparently; probably are passed as copies to apply_async
        for ix, res in enumerate(results):
            capsids[ix], umbrella_windows[ix] = res.get()

        #---------- EXCHANGE HERE -------------    
        #random exchange for now
        #print capsids
        ix1 = np.random.randint(len(capsids)-1)
        ix2 = ix1+1
        E1 = mc.full_energy(capsids[ix1])   
        E2 = mc.full_energy(capsids[ix2])        

        dE = E2 - E1
        print "dE=",dE
        #if true, do the swap
        if np.random.rand() < math.exp((1.0/kT_values[ix1] - 1.0/kT_values[ix2])*dE):


            caps_tmp = capsids[ix1]
            capsids[ix1]=capsids[ix2]
            capsids[ix2]=caps_tmp
    
            capsids[ix1].kT = kT_values[ix1]
            capsids[ix2].kT = kT_values[ix2]
            print "EXCHANGED"
            #print capsids


        #not kosher, may never shift this way
        if (t1*dt_exchange)%umbrella_nsteps==0:
            #    #only shift windows if all initial configuration are available for the next window
            if True not in [umbrella_window.init_conf is None for umbrella_window in umbrella_windows]:
                for ix, umbrella_window in enumerate(umbrella_windows):
                    umbrella_window = umbrella_windows[ix]
                    umbrella_window.shift((umbrella_window.right-umbrella_window.left)/2)
                    capsids[ix] = copy.deepcopy(umbrella_window.init_conf)
                    umbrella_window.init_conf = None
                    umbrella_window.values = []
            ...
            #else:
            #        for i in range(1000):
            #            print "NOT YET", umbrella_window.values#sum([umbrella_window.init_conf is None for umbrella_window in umbrella_windows])
            #        print "cccc ", Nsub, umbrella_window.right, umbrella_window.left, t, umbrella_window.init_conf
            #        print "dddd ", len(umbrella_window.init_conf.subunits), len(capsid.subunits)
            #        print "shifted..."
            #        P_N = np.histogram(umbrella_window.values, bins=np.arange(0,200))
...
def propagate_old(capsid, dt, umbrella_window, run_params, data_root):   
...
def run_old(run_params, data_root='', init_config=None, run_id=None):
    """Example run function. If _id provided, will update entries during run in db."""
    ...
    umbrella_nsteps = 1200000 #number of steps in an umbrella window
