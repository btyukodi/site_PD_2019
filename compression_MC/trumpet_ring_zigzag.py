...
delta = np.linspace(-0.2,0.2,9)
delta = delta[delta<>0]
A = [11.0, 13.0, 15.0, 17.0]
...
for a in A:
    for d in delta:
        params.append((theta, theta, -theta/2+d, a, a, a))
...
#stage the set
set_id = jobs.stage_set(params_template, params_values, run, comments='Trumpet zigzag: (delta, delta, theta). theta=0.483 (N=13 in this geometry)', group_id=group_id)
