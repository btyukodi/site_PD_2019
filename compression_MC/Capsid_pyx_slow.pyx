...
cdef class Edge(object):
    cdef public object vertex_from, vertex_to, parent_subunit 
    cdef public object edge_type
    cdef public dict binding_affinity, bending_modulus, equilibrium_angle
    cdef public double elastic_modulus, equilibrium_length
    def __init__(self, vertex_from, vertex_to, double equilibrium_length ,
                 edge_type='type1', parent_subunit=None,
                double elastic_modulus=1.0, dict bending_modulus={'type1':1.0}, dict equilibrium_angle={'type1': 0.0},
                dict binding_affinity={'type1': 1.0}):
    ...
    cpdef public bint is_surface_edge(self):
        #need more careful check here based on neighbor parent hubs
        cdef Hub hub_from = self.vertex_from.parent_hub
        cdef Hub hub_to = self.vertex_to.parent_hub
        cdef int n_edges_between_hubs = 0
        #!for vertex in hub_from.vertices:
        #!    n_edges_between_hubs +=int(vertex.edge_out.vertex_to.parent_hub ==hub_to)
        for vertex_i in range(len(hub_from.vertices)):
            n_edges_between_hubs += hub_from.vertices[vertex_i].edge_out.vertex_to.parent_hub == hub_to
        for vertex_i in range(len(hub_to.vertices)):
            n_edges_between_hubs += hub_to.vertices[vertex_i].edge_out.vertex_to.parent_hub == hub_from
...
cdef class Hub(object):
    cdef public object vertices
    cdef public object parent_capsid
    def __init__(self):
        self.vertices = []
        parent_capsid=None
    ...
    def split_from(self, vertices_to_split):
