...
for A in np.linspace(3.5,6.5,31):#np.linspace(1,10,10):
    for zs in [-1.0, -3.0]:
            ...
            for ens in range(1):
                ...
                jobs.stage_runs_in_db(params=params['parameters'], run_function=run, 
                    comments='Umbrella Trumpet ring seeded DISC TEST: (theta, theta, -theta/2+delta). theta=0.28 (N=13 in this geometry)',
                    group_id=group_id)
