...
params_template = {'global_params':{'ensemble':2, 'l_fusion':0.5, 'd_max':(2*kT/(elastic_modulus+bending_modulus))**0.5, 
                                    'l_add':3*(2*kT/min(elastic_modulus,bending_modulus))**0.5, 'tmax':5000000, 'dtsave':2000, 'kT_values': [0.7, 0.8, 0.9, 1.0, 1.1, 1.2]},
...
A = [5.0]#ss[3.0, 4.0, 5.0, 6.0]#np.linspace(0.1,5.0, 11)
zs = [-2.0,-3.0,-4.0]#ss[-4.0, -3.0, -2.5, -2.0, -1.0]#, -2.0]
#------- rings --------------------------
theta = 0.2
params = []
...
params_values = {'names':['#THETA1','#THETA2' ,'#THETA3', '#AFFINITY1', '#AFFINITY2', '#AFFINITY3', '#ACTIVITY'],
                 'values':params}
set_id = jobs.stage_set(params_template, params_values, run, comments='parall temp test', group_id=group_id, host='login-00')
